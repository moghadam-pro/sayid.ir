<?php
/**
 * Single Note. Notes are short — the template stays close to the content,
 * no heavy chrome.
 */
get_header();
			...
			<article <?php post_class(); ?>>
				<header class="single-note__header">
					<a class="single-note__back" href="<?php echo esc_url( get_post_type_archive_link( 'sayid_note' ) ); ?>">
						&rarr; <?php esc_html_e( 'همه‌ی یادداشت‌ها', 'sayid-site-core' ); ?>
					</a>
				...
				<?php if ( has_post_thumbnail() ) : ?>
					<div class="single-note__cover"><?php the_post_thumbnail( 'large' ); ?></div>
				<?php endif; ?>
				...
				<?php if ( $source ) : ?>
					<p class="single-note__source">
						<?php esc_html_e( 'منبع:', 'sayid-site-core' ); ?>
						<a href="<?php echo esc_url( $source ); ?>" target="_blank" rel="noopener"><?php echo esc_html( $source ); ?></a>
					</p>
				<?php endif; ?>
			</article>

			<?php echo Sayid_Core_Render::related( get_the_ID() ); // phpcs:ignore ?>
		</div>
	</main>
	<?php
endwhile;
