...
while ( have_posts() ) : the_post();
	...
	$sections = array(
		'challenge' => array( __( 'چالش', 'sayid' ), get_post_meta( $id, 'sayid_challenge', true ) ),
		'context'   => array( __( 'زمینه و بستر', 'sayid' ), get_post_meta( $id, 'sayid_context', true ) ),
		'process'   => array( __( 'فرایند', 'sayid' ), get_post_meta( $id, 'sayid_process', true ) ),
		'decisions' => array( __( 'تصمیم‌های کلیدی', 'sayid' ), get_post_meta( $id, 'sayid_decisions', true ) ),
		'outcome'   => array( __( 'نتیجه', 'sayid' ), get_post_meta( $id, 'sayid_outcome', true ) ),
	);
			...
			<article <?php post_class(); ?>>
				<header class="single-project__header">
					<a class="single-project__back" href="<?php echo esc_url( get_post_type_archive_link( 'sayid_project' ) ); ?>">
						&rarr; <?php esc_html_e( 'همه‌ی کارها', 'sayid' ); ?>
					</a>
					...
					<dl class="single-project__meta">
						<?php if ( $role ) : ?><div><dt><?php esc_html_e( 'نقش', 'sayid' ); ?></dt><dd><?php echo esc_html( $role ); ?></dd></div><?php endif; ?>
						<?php if ( $org ) : ?><div><dt><?php esc_html_e( 'سازمان', 'sayid' ); ?></dt><dd><?php echo esc_html( $org ); ?></dd></div><?php endif; ?>
						<?php if ( $type ) : ?><div><dt><?php esc_html_e( 'نوع پروژه', 'sayid' ); ?></dt><dd><?php echo esc_html( $type ); ?></dd></div><?php endif; ?>
						<?php if ( $start || $end ) : ?><div><dt><?php esc_html_e( 'بازه', 'sayid' ); ?></dt><dd><?php echo esc_html( trim( "$start – $end", ' –' ) ); ?></dd></div><?php endif; ?>
						<?php if ( $tools ) : ?><div><dt><?php esc_html_e( 'ابزارها', 'sayid' ); ?></dt><dd><?php echo esc_html( $tools ); ?></dd></div><?php endif; ?>
						<?php if ( $collab ) : ?><div><dt><?php esc_html_e( 'همکاران', 'sayid' ); ?></dt><dd><?php echo esc_html( $collab ); ?></dd></div><?php endif; ?>
					</dl>

					<?php if ( $external ) : ?>
						<a class="btn btn--ghost" href="<?php echo esc_url( $external ); ?>" target="_blank" rel="noopener">
							<?php esc_html_e( 'نسخه‌ی کامل در پورتفولیو', 'sayid' ); ?>
						</a>
					<?php endif; ?>
				</header>
				...
				<?php if ( $metrics ) : ?>
					<section class="single-project__block single-project__block--metrics">
						<h2><?php esc_html_e( 'متریک‌ها', 'sayid' ); ?></h2>
				...
				<?php if ( ! empty( $gallery ) && is_array( $gallery ) ) : ?>
					<section class="single-project__gallery">
						<?php foreach ( $gallery as $attachment_id ) : ?>
							<?php echo sayid_cover_html( $attachment_id, 'sayid-featured' ); ?>
						<?php endforeach; ?>
					</section>
				<?php endif; ?>
			</article>
